...
int main ()
{
    ...
    if(fullDiagnosis){
        ...
        //Testing upper triangular inversion:
        hessenberg = hess->convertToFullMatrix(Utilities::A_SIZE+1,Utilities::A_SIZE);
        Utilities::givens_rot(hessenberg);
        vector<vector<double> > Hinv(hessenberg);
        Utilities::invertUpperT(hessenberg,Hinv);
        Utilities::printFullMatrix(Hinv);
